...
class BookingCrudRegressionTest extends BaseApiTest {
  ...
  private static final String CREATE_BOOKING_PAYLOAD = "/payloads/create-booking-request.json";
  private static final String PATCH_BOOKING_PAYLOAD = "/payloads/patch-booking-request.json";

  @Test
  @DisplayName("POST /auth should return a valid token")
  void authShouldReturnValidToken() {
    createAuthToken();
  }

  @Test
  @DisplayName("POST /booking should create a booking with expected fields")
  void createBookingShouldReturnBookingWithExpectedFields() {
    createBooking(readResource(CREATE_BOOKING_PAYLOAD));
  }

  @Test
  @DisplayName("GET /booking/{id} should return the created booking")
  void getBookingShouldReturnCreatedBooking() {
    int bookingId = createBooking(readResource(CREATE_BOOKING_PAYLOAD));
    assertCreatedBookingFields(getBooking(bookingId));
  }

  @Test
  @DisplayName("PATCH /booking/{id} should partially update the booking")
  void patchBookingShouldUpdateSelectedFields() {
    String token = createAuthToken();
    int bookingId = createBooking(readResource(CREATE_BOOKING_PAYLOAD));

    patchBooking(bookingId, readResource(PATCH_BOOKING_PAYLOAD), token)
  ...
  @Test
  @DisplayName("DELETE /booking/{id} should remove the booking")
  ...
  private ValidatableResponse assertCreatedBookingFields(ValidatableResponse response) {
    return response
